...
for mds_id in mds_fam_ids:           # 441 iterations
    for row in po_rows:              # Scanning 441 rows EACH time
        if str(row.get('mds_fam_id')) == str(mds_id):
            sum_qty()
```
**Result**: 441 × 441 = **194,481 unnecessary iterations**
...
**Problem**: `TypeError: stat: path should be string, bytes, os.PathLike or integer, not BytesIO.`
- Was using `FileResponse(io.BytesIO(...))` which doesn't work

**Solution**: Changed to `Response(content=...)`
```python
#  WRONG:
return FileResponse(io.BytesIO(pdf_bytes), media_type="application/pdf")
...
```
Priority Score = (100 - Performance%) × Bad Cases Projected

Example:
- Item A: 30% perf × 200 bad cases = 14,000 (HIGH PRIORITY)
- Item B: 70% perf × 50 bad cases = 1,500 (lower priority)
```
...
### Cache Status
- First PDF generation: Full analysis (may take 10-15s for large deliveries)
...
**Goal**: User submits delivery # → processing happens in background → pulls results later
...
# 4. Click "Batch PDF (Problematic Only)"
#    - First time: ~10-15s (full analysis)
#    - Subsequent: <3 seconds (cache hit!)
# 5. Verify PDF shows:
#    - Summary page first
#    - Priority-ranked items (worst first)
#    - Total bad cases projected
```
...
- `CodePuppyDAR/main.py`
  - Fixed O(n²) loop → O(n) lookup dict
  - Fixed PDF endpoint (FileResponse → Response)
  - Removed PO lines table from HTML
  - Removed duplicate load_read_rates() call
  - Added priority ranking to PDF
  - Added PDF caching

- `CodePuppyDAR/insert_pdf.py` (helper script)
- `CodePuppyDAR/new_pdf_endpoint.py` (reference)
...
3. **Check priority ranking**: Ensure items with worst performance/qty impact appear first
